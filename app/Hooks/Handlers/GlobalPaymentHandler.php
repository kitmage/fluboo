...
class GlobalPaymentHandler
{
    public function register()
    {
        ...
        add_filter('fluent_booking/schedule_validation_rules_data', [$this, 'addPaymentMethodValidation'], 10, 3);
    }
    ...
    public function addPaymentMethodValidation($validation, $postedData, $calendarEvent)
    {
        if (Arr::get($postedData, 'source') == 'admin') {
            return $validation;
        }
        ...
        if ($calendarEvent->isPaymentEnabled($duration)) {
            $validation['rules']['payment_method'] = 'required';
            $validation['messages']['payment_method.required'] = __('Please select a valid payment method', 'fluent-booking-pro');
        }
