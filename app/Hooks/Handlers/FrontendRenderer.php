...
class FrontendRenderer
{
    public function register()
    {
        add_action('init', function () {
            ...
            add_filter('fluent_booking/admin_base_url', function ($url, $extension) use ($frontAppUrl) {
                return $frontAppUrl . $extension;
            }, 100, 2);
    ...
    public function renderFullApp()
    {
        if (get_current_user_id()) {
            $hasPermission = PermissionManager::currentUserHasAnyPermission();
            if ($hasPermission) {
                // disable admin bar
                ...
                $content = '<div style="padding: 40px; background: white; text-align: center; margin: 100px auto; max-width: 500px;" class="fbs_no_permission"><p>' . apply_filters('fluent_booking/no_permission_message', __('You do not have permission to view the bookings', 'fluent-booking-pro')) . '</p></div>';
            }
        } else {
