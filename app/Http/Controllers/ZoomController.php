...
class ZoomController extends Controller
{
    public function get(Request $request)
    {
        ...
        // Batch-load users for this page in one query
        $userIds = $metaItems->pluck('object_id')->filter()->unique()->toArray();
        $usersMap = [];
        if ($userIds) {
            foreach (get_users(['include' => $userIds, 'number' => count($userIds)]) as $user) {
        ...
        foreach ($metaItems as $metaItem) {
            $user = $usersMap[$metaItem->object_id] ?? null;
            if ($user) {
                $userData = [
                    'user_id'    => $user->ID,
    ...
    public function save(Request $request, $forcedUserId = 0)
    {
        $info = $request->get('zoom_credentials');

        $userId = (int) $forcedUserId
    ...
    public function addConnectionByCalendarId(Request $request, $calendarId)
    {
        ...
        return $this->save($request, (int) $calendar->user_id);
    }
    ...
    public function disconnectByCalendarId(Request $request, $calendarId)
    {
        ...
        $connectionInfo = $this->getUserInfo($request, (int) $calendar->user_id);

        if (!$connectionInfo || empty($connectionInfo['id'])) {
            return $this->sendError([
                'message' => __('Zoom credentials not found', 'fluent-booking-pro')
            ]);
        }

        return $this->disconnectByConnectId($request, (int) $connectionInfo['id']);
    }

    public function disconnectByConnectId(Request $request, $forcedConnectedId = 0)
    {
        $connectedId = (int) $forcedConnectedId ?: (int) $request->get('connected_id');
