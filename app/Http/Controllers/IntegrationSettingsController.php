...
class IntegrationSettingsController extends Controller
{
    ...
    public function patchRemoteCalendarConflictSettings(Request $request, $calendarId)
    {
        ...
        $meta = $this->findCalendarMeta($calendar, $request->get('meta_id'));

        if (!$meta) {
            return $this->sendError([
                'message' => __('Integration not found for this calendar', 'fluent-booking-pro')
            ], 404);
        }
    ...
    /**
     * Look up a remote-integration Meta record only if it actually belongs
     * to the calendar's owner. Remote-calendar tokens are stored with
     * `object_id = $user_id`, so a body-supplied `meta_id` must match
     * the URL-targeted calendar's `user_id` — otherwise a user with
     * write access to Calendar A could pass another owner's Meta ID and
     * have the action fire against that owner's integration.
     */
    private function findCalendarMeta(Calendar $calendar, $metaId)
    {
        $metaId = (int) $metaId;
        ...
        return Meta::where('id', $metaId)
            ->where('object_id', (int) $calendar->user_id)
            ->first();
    }
