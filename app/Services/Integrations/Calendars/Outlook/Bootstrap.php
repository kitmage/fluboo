...
class Bootstrap extends BaseCalendar
{
    ...
    public function handleAuthCallback()
    {
        ...
        $requiredScopes = [];
        // Verify the scopes
        $returnedScope = (string) ($response['scope'] ?? '');
        if (strpos($returnedScope, 'ndars.ReadWrite') === false) {
    ...
    public function createEvent($config, Booking $booking)
    {
        ...
        if ($user = get_user_by('email', $calendarOwnerEmail)) {
            $calendarOwnerName = Helper::getDisplayNameFromUser($user);
        }

        $mainGuest = [
            'emailAddress' => array_filter([
                'name'    => Helper::getDisplayNameFromUser($booking),
        ...
        $data = [
            ...
            'location'              => [
                'displayName' => (string) ($booking->getLocationAsText() ?? ''),
            ],
    ...
    public function maybeAddOrRemoveGroupMembers($config, $booking, $allGroupBookings, $isRescheduling)
    {
        ...
        foreach ($allGroupBookings as $groupBooking) {
            ...
            $attendees[] = [
                'emailAddress' => [
                    'name'    => Helper::getDisplayNameFromUser($groupBooking),
