...
class Bootstrap extends BaseCalendar
{
    ...
    public function createEvent($config, Booking $booking)
    {
        ...
        if ($booking->getMeta('__google_calendar_event') && (!$booking->isMultiGuestBooking() && !$booking->isRoundRobinBooking())) {
    ...
    public function patchEvent($config, Booking $booking, $updateData, $isRescheduling)
    {
        ...
        $oldEmail = Arr::get($updateData, 'old_email');
        ...
        if ($oldEmail) {
            ...
            $attendees = array_map(function ($attendee) use ($updateData, $oldEmail) {
                if ($attendee['email'] == $oldEmail) {
