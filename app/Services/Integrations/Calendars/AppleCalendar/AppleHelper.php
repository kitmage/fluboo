...
class AppleHelper
{
    ...
    public static function getTravelDuration($event)
    {
        if (empty($event->additionalProperties)) {
            return 0;
        }

        $travelDuration = Arr::get($event->additionalProperties, 'x_apple_travel_duration');

        return DateTimeHelper::getIsoDurationInMinutes($travelDuration);
    }
}
