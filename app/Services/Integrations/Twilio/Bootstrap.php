...
class Bootstrap
{
    ...
    protected function registerBookingHooks()
    {
        ...
        add_action('fluent_booking/after_booking_rescheduled', [$this, 'smsOnBookingRescheduled'], 10, 3);
        ...
        if (Arr::isTrue($notifications, 'booking_conf_attendee.enabled')) {
            ...
            if ($smsSend) {
                do_action('fluent_booking/log_booking_note', [
                    ...
                    'type'        => 'activity',
                    'description' => __('Booking Confirmation SMS has been sent to attendee', 'fluent-booking-pro'),
                    'booking_id'  => $booking->id
                ]);
            }
        }
        ...
        if (Arr::isTrue($notifications, 'booking_conf_host.enabled')) {
            ...
            if ($smsSend) {
                do_action('fluent_booking/log_booking_note', [
                    'title'       => __('SMS Sent Successfully', 'fluent-booking-pro'),
                    'type'        => 'activity',
                    'description' => __('Booking Confirmation SMS has been sent to host', 'fluent-booking-pro'),
                    'booking_id'  => $booking->id
                ]);
            }
        }
        ...
        if ('guest' == $emailTo && Arr::isTrue($notifications, 'reminder_to_attendee.enabled')) {
            ...
            if ($smsSend) {
                do_action('fluent_booking/log_booking_note', [
                    ...
                    'type'        => 'activity',
                    'description' => __('Booking Reminder SMS has been sent to attendee', 'fluent-booking-pro'),
                    'booking_id'  => $booking->id
                ]);
            }
            ...
            if ($smsSend) {
                do_action('fluent_booking/log_booking_note', [
                    'title'       => __('Reminder SMS Sent Successfully', 'fluent-booking-pro'),
                    'type'        => 'activity',
                    'description' => __('Booking Reminder SMS has been sent to host', 'fluent-booking-pro'),
                    'booking_id'  => $booking->id
                ]);
            }
        }
            ...
            if (Arr::isTrue($notifications, 'cancelled_by_host.enabled')) {
                ...
                if ($smsSend) {
                    do_action('fluent_booking/log_booking_note', [
                        ...
                        'type'        => 'activity',
                        'description' => __('Booking Cancellation SMS has been sent to the Attendee', 'fluent-booking-pro'),
                        'booking_id'  => $booking->id
                    ]);
                }
            }
        ...
        if (Arr::isTrue($notifications, 'cancelled_by_attendee.enabled')) {
            ...
            if ($smsSend) {
                do_action('fluent_booking/log_booking_note', [
                    'title'       => __('Cancellation SMS Sent Successfully', 'fluent-booking-pro'),
                    'type'        => 'activity',
                    'description' => __('Booking Cancellation SMS has been sent to the host', 'fluent-booking-pro'),
                    'booking_id'  => $booking->id
                ]);
            }
        }
    ...
    public function smsOnBookingRescheduled(Booking $booking, $oldBooking, $calendarEvent)
    {
        if (!$calendarEvent) {
            return;
        }
        ...
        \as_unschedule_all_actions('fluent_booking/booking_schedule_reminder_sms', [$oldBooking->id, 'host'], 'fluent-booking');
        \as_unschedule_all_actions('fluent_booking/booking_schedule_reminder_sms', [$oldBooking->id, 'guest'], 'fluent-booking');
            ...
            if (Arr::isTrue($notifications, 'rescheduled_by_host.enabled')) {
                ...
                if ($smsSend) {
                    do_action('fluent_booking/log_booking_note', [
                        ...
                        'type'        => 'activity',
                        'description' => __('Booking Rescheduling SMS has been sent to the host', 'fluent-booking-pro'),
                        'booking_id'  => $booking->id
                    ]);
                }
            }
        ...
        if (Arr::isTrue($notifications, 'rescheduled_by_attendee.enabled')) {
            ...
            if ($smsSend) {
                do_action('fluent_booking/log_booking_note', [
                    'title'       => __('Rescheduling SMS Sent Successfully', 'fluent-booking-pro'),
                    'type'        => 'activity',
                    'description' => __('Booking Rescheduling SMS has been sent to the Attendee', 'fluent-booking-pro'),
                    'booking_id'  => $booking->id
                ]);
            }
        }
    ...
    private function clearGroupRemindersForHost($booking, $emailTo)
    {
        ...
        foreach ($otherBookingIds as $otherBookingId) {
            \as_unschedule_all_actions('fluent_booking/booking_schedule_reminder_sms', [$otherBookingId, 'host'], 'fluent-booking');
        }
    }
}
